        ...
        <div className="no-scrollbar -mx-4 flex snap-x snap-mandatory gap-5 overflow-x-auto px-4 pb-4">
          {top.map((m) => {
            ...
            return (
              <article
                key={m.id}
                className="reveal snap-start group relative w-72 shrink-0 overflow-hidden rounded-3xl bg-black/50 backdrop-blur-xl border border-red-900/40 md:w-80 transition-all duration-500 hover:border-red-500/50 hover:shadow-[0_0_30px_rgba(220,38,38,0.2)] hover:-translate-y-2"
              >
                <div className="relative aspect-[4/5] overflow-hidden">
                  {imgSrc ? (
                    ...
                    <div className="h-full w-full bg-gradient-to-br from-red-950 to-black flex items-center justify-center text-6xl">
                      🍽️
                    </div>
                  )}
                  {/* Cinematic red overlay */}
                  <div className="absolute inset-0 bg-gradient-to-t from-black via-black/40 to-transparent opacity-80" />
                  <span className="absolute left-3 top-3 bg-gradient-to-r from-orange-600 to-red-600 text-white text-xs font-bold px-2.5 py-1 rounded-full border border-orange-500/50 shadow-md">
                    Best Seller
                  </span>
                  <div className="absolute bottom-0 left-0 right-0 p-5">
                    ...
                    <h3 className="mt-1 text-xl font-bold text-white group-hover:text-red-300 transition-colors">{m.name}</h3>
                    <div className="mt-3 flex items-center justify-between">
                      <span className="text-lg font-black text-white bg-red-900/80 px-3 py-1 rounded-full border border-red-500/50 shadow-inner">KD {Number(m.price || 0).toFixed(3)}</span>
                      <button
                        onClick={() => add(m)}
                        className="flex h-10 w-10 items-center justify-center rounded-full bg-gradient-to-r from-red-700 to-red-900 border border-red-500/40 text-white transition-all duration-300 hover:scale-110 hover:shadow-[0_0_15px_rgba(220,38,38,0.4)] hover:border-red-400"
