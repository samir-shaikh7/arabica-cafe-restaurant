        ...
        <div className="grid gap-6 lg:grid-cols-2">
          <div className="reveal space-y-4">
            {contactCards.map((c) => (
              <div
                key={c.title}
                className="flex items-start gap-4 rounded-2xl bg-black/50 backdrop-blur-xl border border-red-900/40 p-5 shadow-2xl hover:border-red-500/50 hover:shadow-[0_0_20px_rgba(220,38,38,0.2)] hover:-translate-y-1 transition-all duration-300 group"
              >
                <div className="flex h-12 w-12 shrink-0 items-center justify-center rounded-2xl bg-gradient-to-br from-red-600 to-red-900 text-white shadow-[0_0_10px_rgba(220,38,38,0.4)] border border-red-500/50 group-hover:scale-105 transition-transform">
                  <c.icon className="h-6 w-6 drop-shadow-sm" />
                </div>
                <div>
                  <div className="font-semibold text-white">{c.title}</div>
                  <div className="text-sm text-white/60 mt-0.5">{c.text}</div>
                </div>
              </div>
            ))}
            ...
            <div className="flex flex-wrap gap-3 pt-2">
              ...
              <a
                href={`tel:${callNumber}`}
                className="inline-flex items-center gap-2 rounded-full bg-black/60 border border-red-900/50 px-6 py-3 font-semibold text-white/90 shadow-lg hover:border-red-500/50 hover:text-white hover:shadow-[0_0_15px_rgba(220,38,38,0.3)] backdrop-blur-md transition-all duration-300 group"
              >
