...
export function CartDrawer() {
  ...
  return (
    <div className={`fixed inset-0 z-[60] ${open ? "" : "pointer-events-none"}`}>
      ...
      />
      <aside
        className={`absolute right-0 top-0 flex h-full w-full max-w-md flex-col bg-white shadow-elegant border-l border-amber-100 transition-transform duration-500 ${
          open ? "translate-x-0" : "translate-x-full"
        }`}
      >
        <header className="flex items-center justify-between border-b border-amber-100 p-5">
          <div>
            <h3 className="text-xl font-semibold">Your Order</h3>
            <p className="text-xs text-foreground/60">{items.length} item{items.length === 1 ? "" : "s"}</p>
          </div>
          <button onClick={() => setOpen(false)} className="rounded-full p-2 hover:bg-primary/10" aria-label="Close cart">
            <X className="h-5 w-5" />
          </button>
        </header>
        ...
        <div className="flex-1 overflow-y-auto p-5">
          {items.length === 0 ? (
            <div className="flex h-full flex-col items-center justify-center text-center">
              ...
              <p className="mt-1 text-sm text-foreground/50">Add delicious dishes from our menu.</p>
              <button
                onClick={() => setOpen(false)}
                className="mt-6 rounded-full gradient-primary px-6 py-2.5 text-sm font-semibold text-primary-foreground"
              >
                Browse Menu
              </button>
            </div>
            ...
            <ul className="space-y-3">
              {items.map((i) => (
                <li key={i.id} className="flex gap-3 rounded-2xl bg-amber-50/60 border border-amber-100 p-3">
                  ...
                  <div className="flex-1">
                    <div className="flex justify-between gap-2">
                      ...
                      <button 
                        onClick={() => remove(i.id)} 
                        className="text-foreground/50 hover:text-destructive group/trash" 
                        aria-label="Remove"
                      >
                    ...
                    <div className="mt-2 inline-flex items-center gap-2 rounded-full bg-white border border-amber-100 px-1 py-1">
                      <button
        ...
        {items.length > 0 && (
          <footer className="space-y-3 border-t border-amber-100 p-5 bg-white">
            ...
            <button
              onClick={clear}
              className="w-full rounded-full py-2 text-sm text-foreground/60 hover:text-destructive"
            >
              Clear cart
            </button>
          </footer>
