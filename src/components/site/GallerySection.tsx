      ...
      <section id="gallery" ref={revealRef as React.RefObject<HTMLElement>} className="scroll-mt-24 py-6 md:py-8 section-gallery">
        <div className="mx-auto max-w-7xl px-4">
          <div className="mb-6 text-center">
            <div className="h-6 w-24 rounded-full bg-secondary animate-pulse mx-auto mb-3" />
        ...
        <div className="grid auto-rows-[160px] grid-cols-2 gap-3 md:auto-rows-[200px] md:grid-cols-4 md:gap-4">
          {galleryImages.map((img, i) => {
            ...
            return (
              <figure
                key={img.id || i}
                className={`reveal group relative overflow-hidden rounded-2xl shadow-warm border border-border/50 bg-card ${spanClass}`}
              >
                <img
                  src={imgSrc}
                ...
                />
                <div className="absolute inset-0 bg-gradient-to-t from-foreground/75 via-transparent to-transparent opacity-0 transition group-hover:opacity-100" />
                {img.title && (
