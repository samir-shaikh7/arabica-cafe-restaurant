        ...
        <div className="grid grid-cols-2 gap-3 sm:gap-5 md:grid-cols-3 lg:grid-cols-4 min-h-[400px]">
          {loading ? (
            ...
            filtered.map((m) => {
              ...
              <article
                key={m.id}
                className="group reveal tilt-card relative overflow-hidden rounded-2xl sm:rounded-3xl card-premium flex flex-col"
              >
                <Link to="/product/$id" params={{ id: m.id }} className="relative aspect-[4/3] overflow-hidden shrink-0 block">
                  ...
                  <div className="h-full w-full bg-gradient-to-br from-amber-50 to-orange-50 flex items-center justify-center text-5xl">
                    🍽️
                  </div>
                )}
                  {/* Subtle gradient overlay - light, not dark */}
                  ...
                  <div className="absolute right-2 top-2 sm:right-3 sm:top-3 flex items-center gap-1 rounded-full bg-white/90 border border-amber-100 px-1.5 py-0.5 sm:px-2.5 sm:py-1 text-[10px] sm:text-xs font-bold shadow-soft backdrop-blur-sm">
                    <Star className="h-2.5 w-2.5 sm:h-3 sm:w-3 fill-amber-400 text-amber-400" />
                    <span className="text-foreground/80">{m.rating}</span>
                  </div>
                </Link>

                <div className="p-3 sm:p-5 md:p-5 flex flex-col flex-grow bg-white">
                  <div className="flex items-start justify-between gap-1 sm:gap-2">
                    <Link to="/product/$id" params={{ id: m.id }}>
                      <h3 className="text-sm sm:text-base md:text-lg font-bold leading-tight text-foreground group-hover:text-primary transition-colors">{m.name}</h3>
                    </Link>
                    <div
                  ...
                  <p className="mt-1 sm:mt-2 line-clamp-2 text-[10px] sm:text-sm text-foreground/55 leading-relaxed flex-grow">{m.description}</p>

                  <div className="mt-2 sm:mt-3 flex flex-row items-center justify-between gap-1">
                    <SpiceMeter level={m.spice} />
                    <span className="text-sm sm:text-xl md:text-2xl font-black text-gradient-warm">KD {Number(m.price || 0).toFixed(3)}</span>
                  </div>

                  <div className="mt-3 sm:mt-4 shrink-0">
                    {inCart ? (
                      <div className="flex items-center justify-between rounded-full bg-amber-50 border border-amber-200 p-1 sm:p-1.5 animate-fade-in">
                        <button
                          onClick={() => setQty(m.id, inCart.qty - 1)}
                          className="flex h-7 w-7 sm:h-9 sm:w-9 items-center justify-center rounded-full gradient-primary text-primary-foreground shadow-soft active:scale-90 transition-transform"
                          aria-label="Decrease"
                        >
                          <Minus className="h-3 w-3 sm:h-4 sm:w-4" />
                        </button>
                        <span className="font-bold text-[10px] sm:text-sm text-foreground/80">{inCart.qty} <span className="hidden sm:inline text-foreground/50">in cart</span></span>
                        <button
                          onClick={() => setQty(m.id, inCart.qty + 1)}
                          className="flex h-7 w-7 sm:h-9 sm:w-9 items-center justify-center rounded-full gradient-primary text-primary-foreground shadow-soft active:scale-90 transition-transform"
                          aria-label="Increase"
                      ...
                      <button
                        onClick={() => add(m)}
                        className="flex w-full items-center justify-center gap-1 sm:gap-2 rounded-full gradient-primary py-2 sm:py-3 text-xs sm:text-sm font-bold text-primary-foreground transition-all duration-300 hover:scale-[1.02] hover:shadow-warm active:scale-[0.98]"
                      >
