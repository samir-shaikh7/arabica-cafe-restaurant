...
export function MenuSection() {
  ...
  useEffect(() => {
    ...
    // Set first category as active initially if none is set
    setActiveCategory((prev) => prev ? prev : categories[0].id);

    const observerOptions = {
      root: null,
      rootMargin: "-120px 0px -60% 0px", // triggers when section is in active view area
      threshold: 0,
    };
  ...
  return (
    <section id="menu" ref={revealRef} className="relative scroll-mt-24 py-6 md:py-8 section-menu">
      ...
      <div className="relative mx-auto max-w-7xl px-4">
        ...
        {/* Search + Jump Links */}
        <div className="reveal mb-8 space-y-6">
          ...
          {categories.length > 0 && (
            <div className="flex flex-wrap gap-2.5 justify-center items-center py-2 px-1 max-w-4xl mx-auto">
              {categories.map((c) => (
                <button
                  key={c.id}
                  id={`pill-${c.id}`}
                  onClick={() => scrollToCategory(c.id)}
                  className={`shrink-0 rounded-full px-5 py-2.5 text-xs sm:text-sm font-bold transition-all duration-300 category-pill cursor-pointer hover:scale-[1.03] active:scale-[0.98] ${
                    activeCategory === c.id ? "active" : ""
                  }`}
                >
                  <span className="mr-2 text-sm sm:text-base leading-none select-none">{c.icon}</span>
                  <span>{c.name}</span>
                </button>
              ))}
            </div>
              ...
              >
                ...
                <div className="grid grid-cols-2 gap-3 sm:gap-5 md:grid-cols-3 lg:grid-cols-4">
                  {group.items.map((m) => {
                    ...
                    const minPrice = hasVariants ? Math.min(...(m.variants || []).map((v) => v.price)) : (m.price || 0);
                    return (
                      <article
                        key={m.id}
                      ...
                      >
                        ...
                        <div className="p-3 sm:p-5 md:p-5 flex flex-col flex-grow z-10 relative">
                          ...
                          <div className="mt-2 sm:mt-3 flex items-center justify-between gap-2">
                            ...
                            <span className="text-xs sm:text-sm md:text-base font-black text-primary bg-secondary px-3 py-1.5 rounded-full border border-border/80 shadow-sm whitespace-nowrap">
                              {formatPrice(minPrice)}
                            </span>
                          </div>

                          <div className="mt-3 sm:mt-4 shrink-0 border-t border-border/30 pt-3">
                            {hasVariants ? (
                              <Link
                                to="/product/$id" params={{ id: m.id }}
                                className="flex w-full h-9 sm:h-11 items-center justify-center rounded-full gradient-primary text-xs sm:text-sm font-bold text-white shadow-soft transition-transform hover:scale-[1.02] active:scale-[0.98]"
                              >
                                Options
                              </Link>
                            ) : inCart ? (
                              <div className="flex h-9 sm:h-11 w-full items-center justify-between rounded-full bg-secondary border border-border px-1.5 shadow-inner">
                                <button
                                  onClick={() => setQty(inCart.cartId, inCart.qty - 1)}
                                  className="flex h-7 w-7 sm:h-9 sm:w-9 items-center justify-center rounded-full bg-card hover:bg-muted text-foreground/80 transition-colors shadow-sm"
                                  aria-label="Decrease"
                                >
                                  <Minus className="h-3 w-3 sm:h-4 sm:w-4" />
                                </button>
                                <span className="font-bold text-foreground text-xs sm:text-sm">{inCart.qty} <span className="hidden sm:inline text-foreground/50 font-medium">in cart</span></span>
                                <button
                                  onClick={() => setQty(inCart.cartId, inCart.qty + 1)}
                                  className="flex h-7 w-7 sm:h-9 sm:w-9 items-center justify-center rounded-full gradient-primary text-white shadow-sm hover:scale-105 transition-transform"
                                  aria-label="Increase"
                                >
                              ...
                              <button
                                onClick={() => add(m)}
                                className="flex w-full h-9 sm:h-11 items-center justify-center gap-1.5 rounded-full gradient-primary text-xs sm:text-sm font-bold text-white shadow-soft transition-transform hover:scale-[1.02] active:scale-[0.98]"
                              >
