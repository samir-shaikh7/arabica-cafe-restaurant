  ...
  return (
    ...
    >
      ...
      <div className="reveal relative">
        ...
        <div className="pointer-events-none absolute inset-y-0 left-0 z-10 w-24 bg-gradient-to-r from-[oklch(0.94_0.045_65)] to-transparent" />
        <div className="pointer-events-none absolute inset-y-0 right-0 z-10 w-24 bg-gradient-to-l from-[oklch(0.94_0.045_65)] to-transparent" />
        <div className="flex w-max animate-marquee gap-5">
          {items.map((r, i) => {
            ...
            return (
              <article
                key={`${r.id}-${i}`}
                className="w-80 shrink-0 rounded-3xl bg-white border border-amber-100 p-6 shadow-warm hover:shadow-elegant transition-shadow duration-300"
              >
                <div className="flex items-center gap-1 text-amber-400">
                  ...
                  {Array.from({ length: 5 - displayRatingItem }).map((_, j) => (
                    <Star key={`empty-${j}`} className="h-4 w-4 text-amber-200" />
                  ))}
                </div>
                ...
                <div className="mt-4 flex items-center gap-3">
                  <div className="flex h-10 w-10 items-center justify-center overflow-hidden rounded-full gradient-primary text-sm font-bold text-primary-foreground shadow-soft shrink-0">
                    {imgSrc ? (
                      <img
                        src={imgSrc}
                        alt={displayName}
                  ...
                  <div>
                    ...
                    <div className="text-[11px] text-foreground/50">Verified customer</div>
                  </div>
                </div>
              </article>
            );
