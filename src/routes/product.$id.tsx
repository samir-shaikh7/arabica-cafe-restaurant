      ...
      <div className="min-h-screen bg-atmosphere text-foreground flex flex-col">
        ...
        <main className="flex-1 pt-28 pb-20 px-4 flex items-center justify-center">
          <div className="animate-pulse flex flex-col items-center">
            ...
            <p className="mt-4 text-foreground/50 font-medium">Loading dish...</p>
          </div>
        </main>
        <Footer />
      </div>
      ...
      <div className="min-h-screen bg-atmosphere text-foreground flex flex-col">
        ...
        <main className="flex-1 pt-28 pb-20 px-4 flex items-center justify-center">
          <div className="text-center">
            ...
            <p className="text-foreground/60 mb-8">This dish might have been removed or is currently unavailable.</p>
      ...
      <main className="flex-1 pt-24 pb-20 px-4 sm:px-6">
        <div className="max-w-6xl mx-auto">
          <Link
            to="/"
            hash="menu"
            className="inline-flex items-center gap-2 text-sm font-semibold text-foreground/60 hover:text-primary mb-8 sm:mb-12 transition-colors"
          >
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-10 lg:gap-16 items-start">
            {/* Image Column */}
            <div className="relative aspect-square w-full max-w-2xl mx-auto rounded-[2.5rem] overflow-hidden bg-gradient-to-b from-white to-black/10 shadow-elegant animate-fade-in">
              {(product.image || product.image_url) ? (
                ...
                <div className="h-full w-full bg-amber-50 flex items-center justify-center text-7xl">
                  🍽️
                </div>
              )}
            </div>
            ...
            <div className="flex flex-col animate-fade-in" style={{ animationDelay: "100ms" }}>
              ...
              <div className="flex items-center gap-4 mb-8">
                <div className="flex items-center gap-1.5 rounded-full bg-amber-50 border border-amber-100 px-3 py-1 text-sm font-bold shadow-sm">
                  <Star className="h-4 w-4 fill-amber-400 text-amber-400" />
                  <span className="text-amber-700">{product.rating || "4.5"}</span>
                </div>
                <span className="text-foreground/30">•</span>
                <span className="text-sm font-medium text-foreground/60">Premium Choice</span>
              </div>
              ...
              <div className="flex items-center gap-6 mb-10">
                <div className={`flex items-center gap-2 rounded-full border-2 px-4 py-2 text-sm font-bold ${product.veg ? "border-emerald-500/20 bg-emerald-50 text-emerald-600" : "border-red-500/20 bg-red-50 text-red-600"}`}>
                  {product.veg ? <Leaf className="h-4 w-4" /> : <span className="h-2 w-2 rounded-full bg-red-500" />}
                  {product.veg ? "Vegetarian" : "Non-Vegetarian"}
                </div>
                {(product.spice || 0) > 0 && (
                  <div className="flex items-center gap-2">
                    <span className="text-sm font-bold text-foreground/60">Spice Level:</span>
              ...
              <div className="mt-auto pt-8 border-t border-border/50 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-6">
                ...
                <div className="w-full sm:w-auto">
                  {inCart ? (
                    <div className="flex h-14 sm:h-16 w-full sm:w-48 items-center justify-between rounded-full bg-white border border-border shadow-soft px-2">
                      <button
                        onClick={() => setQty(product.id, inCart.qty - 1)}
                        className="flex h-10 w-10 sm:h-12 sm:w-12 items-center justify-center rounded-full bg-muted text-foreground/70 transition-colors hover:bg-primary/10 hover:text-primary active:scale-95"
                      >
