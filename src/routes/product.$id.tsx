            ...
            <div className="flex flex-col animate-fade-in" style={{ animationDelay: "100ms" }}>
              ...
              {hasVariants ? (
                <div className="mt-auto pt-8 border-t border-border/50">
                  <h3 className="text-xs font-bold uppercase tracking-widest text-foreground/35 mb-4">
                    SELECT OPTION
                  </h3>
                  <div className="space-y-3">
                    {(product.variants || []).map((variant: any) => {
                      const vCartId = `${product.id}-${variant.name}`;
                      const vInCart = items.find((i) => i.cartId === vCartId);
                      return (
                        <div key={variant.name} className="flex items-center justify-between rounded-3xl border border-border bg-card p-4 shadow-sm transition-all hover:border-primary/20 hover:shadow-md">
                          <div>
                            <div className="text-lg font-bold text-foreground">{variant.name}</div>
                            <div className="text-sm font-bold text-primary mt-0.5">{formatPrice(variant.price)}</div>
                          </div>
                          <div>
                            {vInCart ? (
                              <div className="flex h-10 w-28 items-center justify-between rounded-full bg-secondary border border-border px-1.5">
                                <button
                                  onClick={() => setQty(vCartId, vInCart.qty - 1)}
                                  className="flex h-7 w-7 items-center justify-center rounded-full bg-card hover:bg-muted text-foreground/80 transition-colors"
                                >
                                  <Minus className="h-4 w-4" />
                                </button>
                                <span className="font-bold text-foreground text-sm">{vInCart.qty}</span>
                                <button
                                  onClick={() => setQty(vCartId, vInCart.qty + 1)}
                                  className="flex h-7 w-7 items-center justify-center rounded-full gradient-primary text-white shadow-sm hover:scale-105 transition-transform"
                                >
                              ...
                              <button
                                ...
                                className="flex h-10 w-28 items-center justify-center gap-1.5 rounded-full gradient-primary text-sm font-bold text-white shadow-soft transition-transform hover:scale-105 active:scale-95"
                              >
