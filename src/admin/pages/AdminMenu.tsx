...
import { ImageUrlInput } from "@/admin/components/ImageUrlInput";
    ...
    <div className="admin-page">
              ...
              <tbody>
                {filteredItems.map((item) => (
                  <tr key={item.id}>
                    ...
                    <td>
                      <span className="admin-table__price">{formatPrice(item.price as number)}</span>
                    </td>
                    <td>
                      {item.is_available ? (
      ...
      >
        <form onSubmit={handleSave} className="admin-form">
          ...
          <div className="admin-form__group rounded-2xl border border-border bg-card p-5 mt-6 shadow-sm">
            <div className="flex items-center justify-between mb-4">
              <label className="admin-form__label !mb-0 text-foreground/55">PRODUCT VARIANTS (OPTIONAL)</label>
              <button 
                type="button" 
              ...
              <div className="space-y-3">
                {formData.variants.map((variant, index) => (
                  <div key={index} className="flex gap-3 items-center">
                    <input
                      placeholder="Name (e.g. 10 Sticks)"
                      value={variant.name}
                      onChange={e => {
                      ...
                      className="flex-1 w-full rounded-full border border-border bg-background px-5 py-3 text-sm text-foreground focus:border-primary focus:outline-none focus:ring-1 focus:ring-primary transition-colors placeholder:text-foreground/40 shadow-inner"
                    />
                    <input
                      type="number"
                      step="0.001"
                      ...
                      className="w-28 shrink-0 rounded-full border border-border bg-background px-5 py-3 text-sm text-foreground focus:border-primary focus:outline-none focus:ring-1 focus:ring-primary transition-colors placeholder:text-foreground/40 shadow-inner"
                    />
                    <button 
                      type="button" 
                      onClick={() => {
                      ...
                      className="text-foreground/40 hover:text-destructive p-2 transition-colors"
                    >
                      <X className="h-4 w-4" />
                    </button>
                  </div>
                ))}
                <div className="pt-3 text-[10px] text-foreground/45 italic">
                  Note: The base price above is still used as a display fallback if variants exist.
                </div>
              </div>
            ) : (
              <div className="text-sm text-foreground/45 italic py-2">No variants added. The base price above will be used.</div>
            )}
          </div>
